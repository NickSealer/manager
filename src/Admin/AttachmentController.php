...
use Ninja\Manager\Models\Attachment;
use Auth;
use URL;
use File;
use Image;
class AttachmentController extends Controller
{
    public function index(Request $request)
    {
      ...
      return view('manager::admin.attachments.index', compact('attachments', 'isEditor'));
    }
    public function store(Request $request)
    {
      $validatedData = $request->validate([
        'upload' => 'required|mimes:jpeg,bmp,png,gif|max:1000'
      ]);
      ...
      if($request->responseHTML) {
        return redirect()->route('admin.attachments.index', 'CKEditor=content&CKEditorFuncNum=1&langCode=es');
      } else {
